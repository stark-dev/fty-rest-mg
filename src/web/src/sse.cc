...
std::string Sse::changeFtyProtoAlert2Json(fty_proto_t *alert)
{
  ...
  auto asset_element = persist::select_asset_element_web_byName(_connection, fty_proto_name(alert));
  if (asset_element.status != 1)
  {
          ...
          || streq(fty_proto_operation(asset), FTY_PROTO_ASSET_OP_CREATE))
  {
    ...
    if (!tmp.item.ext.empty())
    {
      ...
      for (auto &oneExt : tmp.item.ext)
      {
        ...
        {
          auto oNumber = getNumberOfOutlet(attrName);
    ...
    if (persist::is_rack(tmp.item.basic.type_id))
    {
      ...
      double realpower_nominal = getRackRealpowerNominal(_clientMlm, tmp.item.basic.name.c_str());
...
zmsg_t* Sse::encodeGET(const char* name)
{
...
double Sse::getRackRealpowerNominal(
                                    mlm_client_t *client,
                                    const std::string& name)
{
  double ret = 0.0f;

  zmsg_t *request = encodeGET(name.c_str());
...
std::string Sse::getNumberOfOutlet(const std::string &extAttributeName)
{
