/*  =========================================================================
    fty_rest_selftest.c - run selftests
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_rest_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_rest_selftest.c [options] ...");
    ...
    if (test) {
        ...
        if (!test->subtest)
            ...
            fty_rest_private_selftest (verbose, test->subtest);
#endif // FTY_REST_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
